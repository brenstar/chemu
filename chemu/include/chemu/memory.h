...
//
CHEMU_API extern const uint8_t CHEMU_FONTSET[CHIPMEM_FONTSET_LEN];
...
//
CHEMU_API void chemu_mem_init(ChipMem *mem);
...
//
CHEMU_API uint8_t chemu_mem_read(ChipMem *mem, ChipAddress addr);
...
//
CHEMU_API bool chemu_mem_write(ChipMem *mem, ChipAddress addr, uint8_t value);

//
// Gets the address of the font sprite for the given digit. The digit
// parameter must be in the range of 0 to 15.
//
#define chemu_mem_getFont(digit) (offsetof(ChipMem_reserved, fontset) + (digit * 5))
